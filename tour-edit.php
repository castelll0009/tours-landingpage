...
if (isset($_POST['id'])) {
    ...
    foreach ($days as $day) {
        ...
        // Verifica si se ha subido una nueva imagen days
        if (isset($_FILES['day-previewImage'.$number_day]) && $_FILES['day-previewImage'.$number_day]['size'] > 0) {
            $image = $_FILES['day-previewImage'.$number_day];
            $image_name = $image['name'];        
            // Reemplaza los espacios por guiones bajos en el nombre del archivo
            $image_name = str_replace(' ', '_', $image_name);    
            $image_tmp = $image['tmp_name'];
            ...
            if (move_uploaded_file($image_tmp, $image_path)) {
                // Actualiza la imagen del tour solo si se ha subido una nueva
                $query_dias = "INSERT INTO dias (tour_id, image_path) VALUES ('$new_tour_id', '$image_day')";
                $result_update_image = mysqli_query($connection, $query_dias);

                if (!$result_update_image) {
                    die('Query Failed days image: ' . mysqli_error($connection));
                }
              
            } else {
                echo "days Image upload failed";
            }
        }
        
        // //antes de agregar los datos de los dias vamos a agregar la imagen
        // // Handle image upload for the tour preview
        // $image = $_FILES['dayImage1'];
        // $image_name = $image['name'];
        // $image_tmp = $image['tmp_name'];
        // $image_path = 'imgs/' . $image_name;
